...
const Logo = () => (
  <span className="flex h-6 w-6 items-center justify-center rounded-full border border-brand/30 bg-brand/15 shadow-[0_0_20px_rgba(124,58,237,0.35)]">
    ...
    <header className="fixed top-0 left-0 right-0 z-50">
      <div className={`container mx-auto px-4 transition-all duration-200 ease-out ${scrolled ? 'py-2' : 'py-2'}`}>
        <nav className={`flex items-center justify-between gap-4 rounded-xl transition-all duration-200 ease-out ${
          scrolled 
            ? 'px-4 py-2.5 bg-[#0e0e18]/90 backdrop-blur-2xl backdrop-saturate-150 border border-white/10 shadow-xl shadow-black/40' 
          ...
          <Link href="/" className="flex items-center gap-2.5 text-white hover:text-brand transition-colors">
            <Logo />
            <span className="font-semibold text-sm hidden sm:block">Yunexal Panel</span>
          </Link>
